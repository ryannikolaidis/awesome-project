...
test-cov: install-dev ## Run tests with coverage (auto-installs dev dependencies)
	uv run pytest --cov=awesome_project --cov-report=html --cov-report=term

lint: install-dev ## Run linters (auto-installs dev dependencies)
	uv run ruff check awesome_project tests
	uv run pyright awesome_project

lint-all: install-dev ## Run linters including tests (auto-installs dev dependencies)
	uv run ruff check awesome_project tests
	uv run pyright awesome_project tests

tidy: install-dev ## Fix formatting and linting issues (auto-installs dev dependencies)
	uv run ruff format awesome_project tests
	uv run ruff check --fix awesome_project tests
	uv run black awesome_project tests

check: lint ## Run all checks (alias for lint)

version-dev: ## Bump development version
	@current_version=$$(grep -E "^__version__ = " awesome_project/__init__.py | cut -d '"' -f2); \
	if [[ $$current_version =~ ^([0-9]+)\.([0-9]+)\.([0-9]+)$$ ]]; then \
		...
		sed -i '' 's/__version__ = ".*"/__version__ = "'$$new_version'"/' awesome_project/__init__.py; \
...
uninstall-package: ## Uninstall package from pipx
	pipx uninstall awesome-project
...
docker-build: ## Build Docker image
	docker build -t awesome-project:latest .

docker-run: ## Run Docker container
	docker run --rm -it awesome-project:latest
